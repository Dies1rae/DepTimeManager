                        ...
                        if(count($uniqueData)>0){
                            ...
                            for ($k=0; $k < 7; $k++) { 
                                ...
                                for ($j=0; $j < count($uniqueData); $j++) { 
                                    ...
                                    elseif($uniqueData[$j]['g_uid'] == '6' && ($temp_cellDate >= $temp_startDate) && ($temp_cellDate <= $temp_endDate)){
                                        $testClass->DrawFiveDayGraph();
                                    }
                                    elseif($uniqueData[$j]['g_uid'] == '7' && ($temp_cellDate >= $temp_startDate) && ($temp_cellDate <= $temp_endDate)){
                                        $testClass->DrawFiveDayGraph();
                                    }
                                    elseif($uniqueData[$j]['g_uid'] == '8' && ($temp_cellDate >= $temp_startDate) && ($temp_cellDate <= $temp_endDate)){
                                        $testClass->DrawFiveDayGraph();
                                    }
                                    elseif($uniqueData[$j]['g_uid'] == '9' && ($temp_cellDate >= $temp_startDate) && ($temp_cellDate <= $temp_endDate)){
                                        $testClass->DrawFiveDayGraph();
                                    }
                                    elseif($uniqueData[$j]['g_uid'] == '10' && ($temp_cellDate >= $temp_startDate) && ($temp_cellDate <= $temp_endDate)){
                                        $testClass->DrawFiveDayGraph();
                                    }
                                    // (O_O)///\\\(O_O)
                                    elseif($uniqueData[$j]['g_uid'] == '13' && ($temp_cellDate >= $temp_startDate) && ($temp_cellDate <= $temp_endDate)){
